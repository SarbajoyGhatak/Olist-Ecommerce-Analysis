CREATE OR REPLACE VIEW donut_visual AS
SELECT
    r.seller_id,
    r.total_orders,
    r.revenue_made,
    ...
    r.revenue_rank,
    CASE
        WHEN r.revenue_rank <= 10   THEN 'top_10_sellers'
        WHEN r.revenue_rank <= 100  THEN 'next_90_sellers'
        WHEN r.revenue_rank <= 500  THEN 'mid_tier_sellers'
        ELSE                             'long_tail_sellers'
    END                                                    AS seller_tier,
    CASE
        WHEN r.cancellation_rate_pct >= 10
            THEN 'high_risk'
        WHEN r.revenue_made >= 50000
        AND  r.cancellation_rate_pct <= 2
            THEN 'star_seller'
        WHEN r.stuck_in_transit > r.revenue_made * 0.2
            THEN 'delivery_risk'
        ELSE 'standard'
    END                                                    AS seller_label
FROM (
